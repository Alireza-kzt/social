...
class RecordAudioCubit extends Cubit<RecordaudioState> {
  ...
  void startRecord() async {
    ...
    try {
      final int apiNumber = await Get.find<IDeviceInfo>().getAndroidApiNumber();
      ...
      bool hasStorageAudio = apiNumber >= 33 ? hasAudio : hasStorage;

      if (!hasStorageAudio || !hasMic) {
        if (apiNumber >= 33) {
          if (!hasAudio) await Permission.audio.request();
        } else if (!hasStorage) {
