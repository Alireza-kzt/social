...
class _SendButtonState extends State<SendButton> with TickerProviderStateMixin {
  ...
  late Animation<double> sizeAnimation, posAnimation1, posAnimation2, rotateAnimation;
  late AnimationController recordAnimation, cancelAnimation1, cancelAnimation2, rotationAnimation;
  ...
  void initState() {
    super.initState();
    recordAnimation = AnimationController(duration: const Duration(milliseconds: 100), vsync: this);
    rotationAnimation = AnimationController(duration: const Duration(milliseconds: 400), vsync: this);
    cancelAnimation1 = AnimationController(duration: const Duration(milliseconds: 600), vsync: this);
    cancelAnimation2 = AnimationController(duration: const Duration(milliseconds: 200), vsync: this);

    sizeAnimation = Tween<double>(begin: 1, end: 1.7).animate(CurvedAnimation(parent: recordAnimation, curve: Curves.fastOutSlowIn))
      ..addListener(() {
        sizeListener.value = sizeAnimation.value;
      });
    posAnimation1 = Tween<double>(begin: 0, end: -14).animate(CurvedAnimation(parent: cancelAnimation1, curve: Curves.fastOutSlowIn))
      ..addListener(() {
        micPosListener.value = posAnimation1.value;
      });
    posAnimation2 = Tween<double>(begin: 30, end: -1).animate(CurvedAnimation(parent: cancelAnimation2, curve: Curves.fastOutSlowIn))
      ..addListener(() {
        deletePosListener.value = posAnimation2.value;
      });
    rotateAnimation = Tween<double>(begin: 0, end: 1).animate(CurvedAnimation(parent: rotationAnimation, curve: Curves.fastOutSlowIn));
  ...
  DragAxis? getAxis(DragUpdateDetails details) {
    DragAxis axis;
    _verticalPos = (_verticalPos + details.delta.dy / (context.size!.height * 0.06));
    _horizontalPos = (_horizontalPos + details.delta.dx / (context.size!.width));
    ...
    return Builder(
      builder: (_) {
        final GestureDetector handle = GestureDetector(
          ...
          onPanDown: (_) {
            dragCanceled = false;
            if (dragLocked.value && (context.read<RecordAudioCubit>().state is RecordAudioStarted)) {
                  ...
                  return Stack(
                    children: [
                      if (state is RecordAudioStarted)
                        Container(
                          margin: EdgeInsets.only(right: widget.composerHeight - 12),
                          decoration: BoxDecoration(color: localComposerColor, borderRadius: const BorderRadius.all(Radius.circular(18))),
                          child: Stack(
                            alignment: Alignment.center,
                            children: [
                              if (!dragLocked.value)
                                Align(
                                  alignment: Alignment((value[0] * 0.7) * 2 - 1, 0),
                                  child: Row(
                                    ...
                                    child: Row(
                                      children: [
                                        SizedBox(
                                          ...
                                          child: Stack(
                                            children: [
                                              ...
                                              ValueListenableBuilder<double>(
                                                valueListenable: deletePosListener,
                                                builder: (_, value, __) {
                                                  return Align(
                                                    alignment: Alignment(0, value),
                                        ...
                                        ValueListenableBuilder<Duration?>(
                                          valueListenable: context.read<RecordAudioCubit>().currentDuration,
                                          builder: (_, value, __) {
                                            return Container(
...
class SendType extends StatelessWidget {
  ...
  const SendType({Key? key, required this.height, this.onTap, @required this.icon}) : super(key: key);
